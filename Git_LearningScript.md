...
- 将本地代码托管至Github
如下所示，其中`origin`为后面代码仓库的名称，也可以换成其他名字。`git push -u`表示默认推送分支为master，也可以省略不写手动指定分支，指定默认推送分支以后可以直接简写为`git push`。需要注意的是，如果是异地开发，我们需要在开发之前拉去一下当前分支的代码，保证代码是最新的代码。
  ```
